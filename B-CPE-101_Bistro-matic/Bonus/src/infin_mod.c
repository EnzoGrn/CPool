...
static char *calcul_mod(char *nb1, char *nb2, int neg, char *base)
{
...
char *infin_mod(char *nbr, char *mod, char *base)
{
    int neg = is_negative_mod(nbr, mod);

    clear_values(&nbr, &mod);
    if (mod[0] == '0') {
        write(2, "syntax error", 12);
        exit(EXIT_MOD);
    }
    if (my_strlen(nbr) > 50 || my_strlen(mod) > 50)
        return "0";
    if (strcomp(nbr, mod) < 0)
        return (nbr);
    if (strcomp(nbr, mod) == 0)
        return ("0");
    else
        return calcul_mod(nbr, mod, neg, base);
}
